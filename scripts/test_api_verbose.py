#!/usr/bin/env python3
"""Verbose API test to debug the issue."""
import requests
import json
...
# Build minimal request with just 10 drivers to test
all_drivers = list(projections.items())
test_drivers = all_drivers[:10]

request_body = {
    "salary_cap": 50000,
    "n_drivers": 6,
    "driver_data": [  # Changed from 'drivers' to 'driver_data'
        {
            "driver_id": int(did),
            "name": d['name'],
        ...
        for did, d in test_drivers
    ],
    "objective": "maximize_points",
    "lineup_count": 1
}

print(f"Sending request with {len(test_drivers)} drivers...")
print(f"Request keys: {request_body.keys()}")
print(f"Drivers in request: {len(request_body['driver_data'])}")
